            ...
            using (SqlConnection sqlCon = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    else if (rbWorker.Checked)
                    {
                        ...
                        string query_ID = "SELECT Worker_ID FROM dbo.Worker WHERE NAME = @Name AND WPassKey = @Password";
                        using (SqlCommand cmd = new SqlCommand(query, sqlCon))
                        {
                            cmd.Parameters.AddWithValue("@Name", txtUserName.Text);
                            cmd.Parameters.AddWithValue("@Password", txtPassword.Text);
                            ...
                            {
                                //// Get Worker_ID
                                //SqlCommand cmd_ID = new SqlCommand(query_ID, sqlCon);
                                //cmd_ID.Parameters.AddWithValue("@Name", txtUserName.Text);
                                //cmd_ID.Parameters.AddWithValue("@Password", txtPassword.Text);
                                //int workerId = (int)cmd_ID.ExecuteScalar();
                                //// Redirect to wRequest form
                                //wRequest wRequestForm = new wRequest(workerId);
                                //wRequestForm.Show();
                                //this.Hide();
                                using (SqlCommand cmdId = new SqlCommand(query_ID, sqlCon))
                                {
                                    cmdId.Parameters.AddWithValue("@Name", txtUserName.Text);
                                    cmdId.Parameters.AddWithValue("@Password", txtPassword.Text);
                                    int workerId = Convert.ToInt32(cmdId.ExecuteScalar());
                                    wRequest wRequestForm = new wRequest(workerId); // Pass the retrieved workerId
                                    wRequestForm.Show();
                                    this.Hide();
                                }
                            }
